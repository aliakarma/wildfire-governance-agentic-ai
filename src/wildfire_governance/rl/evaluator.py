...
FIX Issue 5: Changed the FileNotFoundError handler from a silent warning
  (logger.warning + continue) to a proper raise.  Any reproduction run that
  lacks the checkpoint now aborts loudly instead of silently producing PPO
  results from a randomly initialised network.
"""
...
def evaluate(
    ...
    Raises:
        FileNotFoundError: If use_pretrained=True but checkpoint is missing.
    """
    if smoke:
        n_seeds = 2
        grid_size = 10
    ...
    if use_pretrained:
        ...
        # FIX Issue 5: raise instead of silently falling back to random policy
        agent.load_checkpoint(ckpt)  # raises FileNotFoundError if missing
        logger.info("checkpoint_loaded", path=str(ckpt))
    ...
    for seed in range(n_seeds):
        ...
        while not done:
            # Use greedy action selection (no buffer storage) during eval
