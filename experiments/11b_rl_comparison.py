...
FIX Issue 3: Removed the three-line hardcode:
    ppo_cmdp["governance_compliance_pct"] = max(0.0, 92.8 if not smoke else ...)
PPO-CMDP compliance is now measured empirically by running the CMDP agent
with enable_governance=False and auditing the trajectory via
GovernanceInvariantChecker.  The 92.8% figure (or whatever the true value
is) emerges from the experiment, not from source code.
...
def main(config_path: str, smoke: bool = False, use_pretrained: bool = True) -> None:
    ...
    # 3. PPO-CMDP — FIX Issue 3: measured empirically, NOT hardcoded
    # The CMDP agent runs without blockchain enforcement (enable_governance=False).
    # Compliance is measured by GovernanceInvariantChecker, which checks whether
    # every broadcast alert has a valid governance certificate.  Since no certs are
    # issued without the smart contract, the empirical violation rate is 100%
    # non-compliance for ungoverned alerts, which is the correct baseline to compare.
    logger.info("evaluating", method="PPO-CMDP")
    ppo_cmdp = evaluate(
        n_seeds=n_seeds, n_uavs=n_uavs,
        use_pretrained=False, enable_governance=False, smoke=smoke
    )
    # NOTE: ppo_cmdp["governance_compliance_pct"] is now the EMPIRICAL value
    # from GovernanceInvariantChecker, NOT the previously hardcoded 92.8.
    ...
    for seed in range(n_seeds):
        r = run_episode(
            seed=seed, config_name="adaptive_ai",
            n_uavs=n_uavs, n_timesteps=n_timesteps,
