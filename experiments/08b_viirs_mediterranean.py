...
FIX M5: Replaced bare `from _viirs_runner import` with the package-relative
  `from experiments._viirs_runner import` so the script works when invoked
  from the repo root (the standard invocation).
...
def main(config_path: str, smoke: bool = False) -> None:
    # FIX M5: use package-relative import, not bare module name
