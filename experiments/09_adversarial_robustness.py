...
FIX Issue 4: Byzantine fault rows now use cfg.blockchain.max_byzantine (=2)
  rather than max(0, 7//3 - 1) = 1.  This ensures the canonical scenario
  k=7, f=2 is used throughout, making the code, the paper, and the CSV
...
def main(config_path: str, smoke: bool = False) -> None:
    ...
    try:
        ...
        # FIX Issue 4: load canonical max_byzantine from config (default=2)
        max_byzantine = cfg.blockchain.max_byzantine
        n_validators = cfg.blockchain.n_validators
        p_c = 0.3  # per-validator compromise probability for Theorem 2
    except Exception:
    ...
    for atk in ATTACK_CONFIGS:
        ...
        for seed in range(n_seeds):
            ...
            n_inject_blocked += r_gomdp.n_injections_blocked
        ...
        # Theoretical breach probability (FIX Issue 4: use cfg.blockchain.max_byzantine)
        if attack_type == "injection":
            p_breach_gomdp = 0.000
            p_breach_central = compute_breach_probability_centralized(1.0)
        elif attack_type == "byzantine":
            # Use the ACTUAL n_byzantine for the row, and canonical max_byzantine for bound
            row_f = n_byzantine  # actual f being tested
            # P_breach for this f level (only defined when f <= max_byzantine)
            if row_f <= (n_validators - 1) // 3:
                p_breach_gomdp = compute_breach_probability_gomdp(
                    n_validators, row_f, p_c
                )
            else:
                # f=3 exceeds the safety threshold → breach probability > threshold bound
                try:
                    p_breach_gomdp = compute_breach_probability_gomdp(
                        n_validators, row_f, p_c
                    )
